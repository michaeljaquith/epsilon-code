...
The aggregate is summarized in Table~\ref{tab:token-focus}.
...
stages the same example as the paper's motivating problem statement
--- the fluent surface, the hidden top-$k$ split, and the
$10$-of-$98$-token review surface --- in three panels.
...
filter into an end-to-end system with usable precision. The reviewer
is given the function plus the specific high-\eps tokens, and asked
to evaluate those tokens in context. At current API prices this
costs fractions of a cent per entry and completes in seconds.
