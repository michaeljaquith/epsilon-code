...
\textsc{high} prompts are API/version-split tasks (Stripe, OpenAI,
SQLAlchemy, FastAPI, Pydantic) where an incorrect choice causes
functional failure. \textsc{low} prompts are pure-logic tasks (sort,
palindrome, Fibonacci, two-sum) where any correct algorithm is
functionally equivalent. The split and detection rates per category
appear in \S\ref{subsec:calibration}.
...
\texttt{stripe.PaymentIntent.create()} (prob.\ $0.47$), $\eps =
0.73$ at the attribute token. This is the target signal.
...
\begin{tabular}{lc}
\toprule
\textbf{Configuration} & \textbf{\textsc{low} FP rate} \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Successive filtering stages on $10$ \textsc{low} prompts.
Naming-convention prompt and AST filter are partially overlapping;
without the prompt, AST filtering alone drops FP from $90\%$ to
$70\%$. Residual $50\%$ is Type 3, rare in production API-integration
code.}
\label{tab:filtering}
\end{table}
...
within the flagged region and also the more precise band at the
token level (\S\ref{subsec:token-focus}). These are UX signals, not
...
flag threshold; $\rho_{0.65}$ is the fraction that exceed the paused
threshold.\footnote{We report $\rho$ at the flagged boundary ($0.30$)
and at $0.65$ (the boundary above which the stage-$2$ reviewer
treats the token as a high-urgency paused signal in production).
Results at $0.95$ are similar in shape with smaller token counts.}
...
\toprule
 & \textbf{Fn-level} & \textbf{\eps $>0.30$} & \textbf{\eps $>0.65$}
   & \textbf{Tokens@flag} & \textbf{Tokens@paused} \\
\textbf{Set}
 & \textbf{flagged} & \textbf{(tokens)} & \textbf{(tokens)}
 & \textbf{per fn} & \textbf{per fn} \\
\midrule
\textsc{high} ($n=104$) & $97.1\%$ & $10.5\%$ & $1.5\%$ & $10.4$ & $1.5$ \\
\textsc{low} ($n=40$)   & $90.0\%$ & $13.1\%$ & $2.4\%$ & $10.4$ & $1.9$ \\
\bottomrule
\end{tabular}
...
flag threshold and $1.5\%$ at the paused threshold. Average function
size is $\sim 98$ retained semantic tokens. Token review burden
reduction: $89\%$ at the flag threshold, $98.5\%$ at the paused
threshold.}
...
identified as high-entropy --- and a $1$-$2$-token problem at the
paused threshold. The function-level flag rate of $97\%$ is not a
problem to be defended against; it is the entry point to a
localization step that no other signal in the literature provides.

\paragraph{Why \textsc{low} looks similar.}
\textsc{low} prompts show a similar token-level focus profile
($13.1\%$ flag, $2.4\%$ paused). The $90\%$ function-level flag rate
on \textsc{low} prompts is the residual Type 3 false-positive floor:
all $30$ \textsc{low} functions executed correctly under
\texttt{exec()} assertions, so the function-level fires are correct
code that \eps observed hesitation on. At the token level, those
fires localize to the $1$--$2$ tokens where a genuinely-alternative-%
but-equivalent choice was available (brute vs.\ hash two-sum;
recursive vs.\ iterative Fibonacci), and the stage-$2$ reviewer
clears them fast because both branches pass.

Figure~\ref{fig:token-focus-detail} shows the token-level \eps
trajectory on Stripe Scenario A (GPT-4o). Of $98$ retained tokens,
$10$ exceed the flag threshold and $2$ exceed the paused threshold.
The $2$ paused tokens are the \texttt{.Ch} method selector on
\texttt{stripe} (the Charge-vs-PaymentIntent decision) and one
immediately adjacent token. The remaining $96$ tokens are confident
parameter construction, imports, and return-value handling. A
reviewer given this figure does not re-read the function; they look
at two tokens.
...
retained tokens). The flag threshold ($0.30$) is dashed; the paused
threshold ($0.65$) is solid. Ten tokens exceed the flag threshold;
two exceed the paused threshold. The paused tokens localize to the
method-selector decision between \texttt{Charge} and
\texttt{PaymentIntent}. The rest of the function is confident
boilerplate. \eps does not just flag the function --- it names the
two tokens that drove the flag.}
