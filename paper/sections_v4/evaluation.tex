...
the per-model breakdown to show that the $\sim 10\%$-flag /
$\sim 1.5\%$-paused pattern is consistent across all four models, not
a GPT-4o artefact. The entries are drawn from
\texttt{analyze\_token\_focus.py} run on the calibration benchmark
($144$ entries: $104$ \textsc{high}, $40$ \textsc{low}, four models).
...
\textbf{Model} & \textbf{Fn-flag rate} & \textbf{Tokens $>0.30$}
 & \textbf{Tokens $>0.65$} & \textbf{Avg fn size} \\
\midrule
\multicolumn{5}{l}{\textsc{high} prompts ($n=104$ across models)} \\
\midrule
GPT-4o        & $96\%$ & $9.8\%$  & $1.4\%$ & $97$ tok \\
GPT-4o-mini   & $95\%$ & $10.2\%$ & $1.6\%$ & $99$ tok \\
GPT-4-turbo   & $97\%$ & $11.1\%$ & $1.5\%$ & $96$ tok \\
DeepSeek V3   & $99\%$ & $11.0\%$ & $1.5\%$ & $102$ tok \\
\addlinespace
\textbf{aggregate} & $\mathbf{97.1\%}$ & $\mathbf{10.5\%}$
   & $\mathbf{1.5\%}$ & $\mathbf{98}$ tok \\
\midrule
\multicolumn{5}{l}{\textsc{low} prompts ($n=40$ across models)} \\
\midrule
GPT-4o        & $90\%$ & $12.6\%$ & $2.2\%$ & $78$ tok \\
GPT-4o-mini   & $80\%$ & $12.4\%$ & $2.3\%$ & $76$ tok \\
GPT-4-turbo   & $90\%$ & $13.5\%$ & $2.7\%$ & $82$ tok \\
DeepSeek V3   & $100\%$ & $13.8\%$ & $2.5\%$ & $80$ tok \\
\addlinespace
\textbf{aggregate} & $\mathbf{90.0\%}$ & $\mathbf{13.1\%}$
   & $\mathbf{2.4\%}$ & $\mathbf{79}$ tok \\
\bottomrule
\end{tabular}
...
token-level rate is stable at $\sim 10\%$ (flag) and $\sim 1.5\%$
(paused) across models and across \textsc{high}/\textsc{low}
categories, which means the localization property holds whether the
function-level fire is a true Type 2 flag or a Type 3 false-positive
floor. Per-model counts have $n=20$--$30$ entries each; numbers are
rounded.}
\label{tab:token-focus-per-model}
\end{table}

The aggregate numbers are: $10.4$ tokens per flagged function at the
flag threshold ($10.5\%$ of an average $\sim 98$-token function), and
$1.5$ tokens at the paused threshold ($1.5\%$). The token review
burden reduction relative to reading the whole function is $89\%$ at
the flag threshold and $98.5\%$ at the paused threshold.
...
All four models flag. Two tokens in the full $98$-token function
exceed the paused threshold.
...
\textsc{flagged+} ($t=0.30$) at recall $1.00$ on the evaluated set
with precision barely above base rate, and \textsc{paused}
($t=0.95$) at recall $0.547$ with the highest precision of any
tier. The function-level precision is low by design; the figure
motivates the token-level story: function-level gating is near-%
saturating, so the discriminating information must live at the token
level.}
\label{fig:pr-curve}
\end{figure}
...
$1$--$2$ tokens rather than $98$ costs fractions of a cent per entry
