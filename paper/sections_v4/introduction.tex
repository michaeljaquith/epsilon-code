...
The gap between a $0.52$-vs-$0.47$ internal split and a confident-%
looking output is not an accident of any particular model; it is
structural. Multi-sample uncertainty quantification methods for LLMs
are \emph{inter-generational}: they sample the same prompt multiple
...
with a $0.52$ prior on the deprecated \texttt{Charge} API samples that
branch nearly every time; once the early tokens commit, the rest of
the function is forced by context. The uncertainty exists, but it
...
our benchmark. A reviewer asked ``which functions need scrutiny''
...
the flag threshold and only $1.5\%$ exceed the paused threshold. The
same score that flags the function also tells the reviewer that the
relevant uncertainty lives in the method selector on line $4$, not in
the parameter block on lines $7$--$12$. \eps is a two-level signal: a
function-level filter that agrees with \papg on ``which,'' and a
token-level localizer that no sequence-level aggregate can produce.
...
semantic tokens, and the model's uncertainty concentrates at $2$ of
them. The reviewer does not need to re-read the function; it needs to
look at two tokens.
...
$10$ tokens ($10.2\%$) and the paused threshold at $2$ tokens
($2.0\%$). The function is flagged --- but the within-function
review surface is a small fraction of the generated code. Darker
intensity $= $ higher \eps. See \S\ref{subsec:token-focus}.}
...
mean token probability \papg~\cite{spiess2025calibration} detects
$65$--$90\%$ of HIGH prompts --- tying \eps on GPT-4o, beating it on
GPT-4o-mini and GPT-4-turbo, and losing to it on DeepSeek V3. \papg
deserves credit: on raw function-level detection it is competitive
with \eps and at the same cost. What it cannot do is identify the
specific token at which the model was uncertain. \eps produces both
the function-level score and the token-level attribution. At
...
Stripe Scenario A, the $2$ tokens whose \eps exceeds the paused
...
\begin{enumerate}[noitemsep]
    ...
    $\sim 98$-token functions reduce to an average of $10.4$
    actionable tokens at the flag threshold ($10.5\%$ of the function)
    and $1.5$ tokens at the paused threshold ($1.5\%$) --- an
    $89$--$98.5\%$ reduction in within-function review burden. The
    function-level flag agrees with \papg; the token-level attribution
    is what no sequence-level signal can produce.
    \item \papg credited: the sequence-level mean token probability
    of Spiess et al.~\cite{spiess2025calibration} ties or beats \eps
    on raw function-level detection across three of four models at
    the same $1\times$ API cost. \eps's case rests on what \papg
    cannot do, not on a rate difference.
