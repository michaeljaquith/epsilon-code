...
\textsc{paused}. The model generated v0
...
design. \textsc{paused} prompts the developer to examine the function,
encountering the deprecated call. This is indirect detection: \eps
flagged adjacent uncertainty, not the failing token itself.
...
\subsection{Review loop: 2A vs.\ 2B}
...
\textbf{2A} uses no reviewer context; \textbf{2B} injects model-%
specific learned FP patterns from a prior clean run (injecting suspect
...
\begin{tabular}{lrr}
\toprule
Metric & 2A (no context) & 2B (with FP context) \\
\midrule
...
that was in fact correct on manual inspection. 2B is the preferred
...
\caption{Injecting learned FP patterns into the reviewer shifts
$31$ additional entries from consolidation to cleared and eliminates
late false positives, at the cost of suppressing some true failures
the 2A reviewer forwarded.}
...
set. 2B's zero late-FP result rests on $n=57$ confirmed entries and
...
\subsection{Ground-truth disclosure}
\label{subsec:gt-disclosure}

LOW ground truth is \texttt{exec()}-verified; HIGH is LLM-judged via
GPT-4o. This includes a circular case where GPT-4o-mini output is
reviewed by a larger model in the same family. We disclose rather
than overclaim human verification we did not perform.
