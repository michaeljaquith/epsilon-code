...
($0.30 \le \eps < 0.95$), \textsc{paused} ($\eps \ge 0.95$). An
earlier version of this system used a four-tier scheme that split
\textsc{flagged} and \textsc{paused} at $0.65$, but both tiers
triggered identical system behavior --- routing to the same parallel
reviewer --- so we merged them. \textsc{paused} is now reserved for
genuinely extreme uncertainty ($\eps \ge 0.95$). Whether \textsc{%
paused} actually holds code delivery until the review completes, or
simply runs the review loop like \textsc{flagged}, is a deliberate
design choice left to the implementer; this paper notes the
possibility but does not prescribe it. These are UX signals, not
calibrated probabilities --- the \textsc{flagged} empirical failure
rate is $\sim 27\%$ (\S\ref{sec:evaluation}); \textsc{paused} fires
rarely enough in our evaluation that its failure rate is a small-%
sample estimate. The load-bearing boundary is the binary
\textsc{complete} vs.\ \textsc{flagged+}: everything \textsc{flagged+}
is forwarded to the stage-2 reviewer. In our qualitative scenarios
A--D across four models, no HIGH prompt crossed the \textsc{paused}
threshold; \textsc{paused} is a design boundary that exists but was
not exercised on the scenarios in this work.
