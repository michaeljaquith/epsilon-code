...
\eps; \eps operates in the gap where rules do not yet exist. The
method requires only top-$k$ log-probabilities, available via
...
expose per-token log-probabilities. The method requires only top-$k$
logprobs --- no model-internal access.
...
token-level precision. The review loop in
