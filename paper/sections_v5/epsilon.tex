...
($13.1\%$ flag). The $90\%$ function-level flag rate on \textsc{low}
prompts is the residual Type 3 false-positive floor: all $30$
\textsc{low} functions executed correctly under \texttt{exec()}
assertions, so the function-level fires are correct code that \eps
observed hesitation on. At the token level, those fires localize to
the handful of tokens where a genuinely-alternative-but-equivalent
choice was available (brute vs.\ hash two-sum; recursive vs.\
iterative Fibonacci), and the stage-$2$ reviewer clears them fast
because both branches pass.
